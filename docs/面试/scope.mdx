...
function test() {
    ...
    a = 100; // 给 a 赋值 100
    console.log(this.a); // 打印this.a， 那么this 执行谁？ 那要看谁调用的test，我们看看：👀test() 谁调用的？ 全局调用的，所以this.a 是外层的 10
    console.log(a); // 打印变量 a 先在当前函数作用域中查找变量 a 我们找一下 🔍能找到嘛，可以！ 有一个 100 所以 第三个是 100
}
