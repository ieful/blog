...
```js
// 题解：这道题没什么难道，只要知道 Vue2 中的 Object.defineProperty 加一点 细心 就能做对

// 定义了一个全局的 option 对象
var option = {
    // 还专门把 data 设置为一个 对象，并且包在 option 中够不够 "心机" ？ 读到这儿我就知道这是有意为之！
    data: {
        a: 1
    }
}
...
class Component {
    constructor(opt) {
        this.data = opt.data; // 重点来了，在constructor 中直接把 opt.data 赋值给了 this.data, 而opt.data 是一个引用类型！！记住后面要考！
        Object.defineProperty(this.data, 'a', {
            get: function () {
...
c1.data.a = 3; // c1 开始操作自己的data了，赋值 3 ，打印： set val： 3
c2.data.a = 5; // c2 开始操作自己的data了， 赋值 5， 打印：set val: 5  由于二者指向的data 其实是同一块内存，所以其实c2改写了 c1 赋的值
